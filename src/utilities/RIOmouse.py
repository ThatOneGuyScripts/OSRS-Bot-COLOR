...
from utilities.ScreenToClient import screen_to_window
...
class Mouse:
    ...
    def move_to(self, destination: tuple, **kwargs):
        if self.RemoteInputEnabled == True:
            ...
            dest_x, dest_y = screen_to_window(destination[0], destination[1])

            start_x, start_y = self.rio.get_current_position()
            for curve_x, curve_y in HumanCurve(
                ...
                self.rio.Mouse_move(int(curve_x), int(curve_y))  # Convert curve_x and curve_y to integers
                self.CurX, self.CurY = int(curve_x), int(curve_y)  # Convert curve_x and curve_y to integers and update class variables
        else:
            """
            Use Bezier curve to simulate human-like mouse movements.
            Args:
    ...
    def move_rel(self, x: int, y: int, x_var: int = 0, y_var: int = 0, **kwargs):
        if self.RemoteInputEnabled == True:
            ...
            self.move_to((self.rio.get_current_position()[0] + x, self.rio.get_current_position()[1] + y), **kwargs)
        else:
            """
            Use Bezier curve to simulate human-like relative mouse movements.
            Args:
    ...
    def click(self, button="left", force_delay=False, check_red_click=False) -> tuple:
        if self.RemoteInputEnabled == True:
                ...
                """
                mouse_pos_before = self.rio.get_current_position()
                x, y = self.rio.get_current_position()
                self.rio.click(x, y)
                mouse_pos_after = self.rio.get_current_position()
                if check_red_click:
    ...
    def __calculate_knots(self, destination: tuple):
        if self.RemoteInputEnabled == True:
            ...
            distance = np.sqrt((destination[0] - self.rio.get_current_position()[0]) ** 2 + (destination[1] - self.rio.get_current_position()[1]) ** 2)
    ...
    def send_modifer_key(self,ID,key):
        self.rio.send_modifier_key(ID,key)
        #ex self.mouse.send_modifer_key(400,'shift')
    
    def send_key(self, ID, KeyChar):
        self.rio.send_key_event(ID,KeyChar)
        #ex self.mouse.send_key(400, 'a')
        
    def send_arrow_key(self,ID,key):
        self.rio.send_arrow_key(ID,key)
        #ex self.mouse.send_modifer_key(400,'left')    
